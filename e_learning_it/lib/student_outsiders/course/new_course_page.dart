...
class CourseNewPage extends StatelessWidget {
  ...
  Future<List<Course>> fetchNewCourses() async {
    final response = await http.get(Uri.parse('http://localhost:3006/api/show_courses?sortBy=newest&limit=3')); // <-- แก้ไขตรงนี้
    if (response.statusCode == 200) {
      final List<dynamic> courseData = json.decode(response.body);
      return courseData.map((json) => Course.fromJson(json)).toList();
    } else {
      throw Exception('Failed to load new courses');
    }
  }
